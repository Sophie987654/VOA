...
class UI_People(object):
    def setupUI(self):
        ...
        if gVar.mode == "touch":
            self.backBtn.clicked.connect(self.go_to_selTrain)
        ...
        for p_type in self.personnel.keys():
            ...
            plus_btn.setFixedSize(int(h*0.05), int(h*0.05))
            plus_btn.setSizePolicy(exExSP)
            plus_btn.setStyleSheet(
                largeFont)
            if gVar.mode == "touch":
                plus_btn.clicked.connect(lambda _, p_type=p_type: self.increase_personnel(p_type))
            elif gVar.mode == "voice":
                plus_btn.installEventFilter(self)

            minus_btn = QPushButton("-")
            minus_btn.setFixedSize(int(h*0.05), int(h*0.05))
            minus_btn.setSizePolicy(exExSP)
            minus_btn.setStyleSheet(
                largeFont)
            if gVar.mode == "touch":
                minus_btn.clicked.connect(lambda _, p_type=p_type: self.decrease_personnel(p_type))
            elif gVar.mode == "voice":
                minus_btn.installEventFilter(self)
            ...
            p_count_label.setAlignment(Qt.AlignCenter)
            ...
            self.btnObjs.append((p_label, minus_btn, p_count_label, plus_btn))
        ...
        if gVar.mode == "touch":
            self.checkBtn.clicked.connect(self.go_to_selSit)
        elif gVar.mode == "voice":
        ...
        for i, (p_label, minus_btn, p_count_label, plus_btn) in enumerate(self.btnObjs):
            self.gridLayout.addWidget(p_label, i+4, 0)
            self.gridLayout.addWidget(minus_btn, i+4, 1)
            self.gridLayout.addWidget(p_count_label, i+4, 2)
            self.gridLayout.addWidget(plus_btn, i+4, 3)
